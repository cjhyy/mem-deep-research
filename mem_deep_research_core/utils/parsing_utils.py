...
def _try_parse_with_pattern(raw_str, pattern):
    ...
    try:
        ...
        for i, key_match in enumerate(key_matches):
            ...
            value_end_pos = _find_value_end_position(raw_str, value_content_start, search_limit)
...
def _find_value_end_position(raw_str, start_pos, search_limit):
    ...
    for pos in range(search_limit - 1, start_pos, -1):
        if raw_str[pos] == '"':
            ...
            if after_quote.startswith(",") or after_quote.startswith("}") or after_quote == "":
                return pos
    return None
...
def parse_llm_response_for_tool_calls(llm_response_content_text, _fix_depth: int = 0):
    ...
    for pattern in incomplete_patterns:
        matches = re.findall(pattern, llm_response_content_text, re.DOTALL | re.IGNORECASE)
    ...
    if bad_tool_calls:
        ...
        for bad_call in bad_tool_calls:
            logger.debug(f"Invalid tool call: {bad_call['error']} - {bad_call['content'][:100]}...")
        ...
        if item["error"] == "Unclosed arguments tag" and _fix_depth < 1:
            ...
            if content.find("<arguments>") != -1 and content.find("</arguments>") == -1:
                ...
                if next_tag != -1:
                    ...
                    fixed_content = content[:next_tag] + "</arguments>" + content[next_tag:]
                else:
