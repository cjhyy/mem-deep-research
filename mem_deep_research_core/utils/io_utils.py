...
def process_input(task_description, task_file_name):
    ...
    if task_file_name:
        ...
        logger.info(f"Info: Detected {file_type} file {task_file_name}, added hint to description.")
...
class OutputFormatter:
    def __init__(self, max_tool_result_chars: int = 30_000):
        """
        Args:
            max_tool_result_chars: Max chars per tool result. Default 30k (~7.5k tokens).
                                   Use -1 for legacy 100k limit.
        """
    ...
    def format_tool_result_for_user(
        self, tool_call_execution_result, max_tool_result_chars: int = None
    ):
        """
        ...
        Args:
            tool_call_execution_result: Tool execution result dict
            max_tool_result_chars: Maximum chars per tool result. If None, uses self.max_tool_result_chars.
                                   Set to -1 to use legacy 100k limit.
        """
        ...
        if "error" in tool_call_execution_result:
            ...
            max_len = 100_000 if effective_limit == -1 else max(10_000, effective_limit)
            if len(content) > max_len:
                content = (
                    content[:max_len]
                    + f"\n... [Result truncated from {len(tool_call_execution_result['result'])} to {max_len} chars]"
                )
        else:
    ...
    def format_final_summary_and_log(self, final_answer_text, client=None):
        ...
        if boxed_result:
            ...
            boxed_result = "Final response is generated by LLM, but no \\boxed{} content found."
        else:
            summary_lines.append("No \\boxed{} content found.")
            boxed_result = FALLBACK_NO_ANSWER
