...
# HITL: Sub-agents disable async HITL degradation (only sync-wait allowed).
# wait_for_human() checks this ContextVar to force the synchronous-only path.
_is_sub_agent_var: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "_is_sub_agent", default=False
)


def save_sub_agent_contextvar_state() -> dict:
    """Snapshot the _is_sub_agent_var value for RuntimeSnapshot."""
    return {"is_sub_agent": _is_sub_agent_var.get(False)}


def restore_sub_agent_contextvar_state(state: dict) -> None:
    """Restore the _is_sub_agent_var value from a RuntimeSnapshot."""
    if "is_sub_agent" in state:
        _is_sub_agent_var.set(state["is_sub_agent"])
...
class SubAgentRunner:
    ...
    async def _run_inner(
        ...
        *,
        ...
        # Mark this execution context as a sub-agent so Runtime.wait_for_human()
        # forces the strictly-synchronous path (design doc: Phase 2 does not
        # support durable HITL inside sub-agents). The ContextVar is scoped
        # to this coroutine's Context; reset on exit so the parent returns
        # to normal HITL behaviour.
        sub_agent_token = _is_sub_agent_var.set(True)
        ...
        try:
            ...
            # HITL durable-suspend signal from inside a sub-agent must bubble
            # to the parent main loop (Phase 2 catches there). Phase 1 design
            # keeps sub-agents on the strictly-synchronous path, so this
            # branch is mostly defensive — but silencing it would turn a
            # misconfiguration into a silent stall.
            raise
            ...
            _is_sub_agent_var.reset(sub_agent_token)
    ...
    async def spawn(
        ...
        *,
        ...
        spawn_sub_agent_token = _is_sub_agent_var.set(True)
        try:
            ...
            # HITL signal — bubble to parent main loop, do not degrade to a
            # string answer. See _run_inner for the matching raise.
            raise
        except Exception as e:
            ...
            _is_sub_agent_var.reset(spawn_sub_agent_token)
